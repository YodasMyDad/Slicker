 ...
 */
export function setSlideClasses(index) {
    const _ = this;
...
export function startLoad() {
    ...
    if (_.options.arrows === true && _.slideCount > _.options.slidesToShow) {
        if (_.$prevArrow)
            DOM.setCSS(_.$prevArrow, { display: 'none' });
        if (_.$nextArrow)
            DOM.setCSS(_.$nextArrow, { display: 'none' });
    }
    if (_.options.dots === true && _.slideCount > _.options.slidesToShow) {
        if (_.$dots)
            DOM.setCSS(_.$dots, { display: 'none' });
    }
 ...
 * Autoplay iterator
 */
export function autoPlayIterator() {
    const _ = this;
 ...
 */
export function lazyLoad(_) {
    // Implemented in lazy loading module
}
export function progressiveLazyLoad(_, _tryCount) {
    // Implemented in lazy loading module
}
export function getLeft(_, _slideIndex) {
    // Implemented in slide-logic module
    return 0;
}
export function slideHandler(_, _index, _sync, _dontAnimate) {
    // Implemented in slide-logic module
}
