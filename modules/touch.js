 ...
 */
export function swipeHandler(event) {
    var _a;
    const _ = this;
    ...
    const actionData = ((_a = event.data) === null || _a === void 0 ? void 0 : _a.action) || event.action;
    switch (actionData) {
        case 'start':
            _.swipeStart(event);
            break;
 ...
 * Swipe start
 */
export function swipeStart(event) {
    const _ = this;
    _.interrupted = true;
 ...
 * Swipe move
 */
export function swipeMove(event) {
    const _ = this;
    const originalEvent = event.originalEvent || event;
    const touches = originalEvent.touches;
    ...
    _.touchObject.swipeLength = Math.round(Math.sqrt(Math.pow((_.touchObject.curX || 0) - (_.touchObject.startX || 0), 2)));
    const verticalSwipeLength = Math.round(Math.sqrt(Math.pow((_.touchObject.curY || 0) - (_.touchObject.startY || 0), 2)));
 ...
 * Swipe end
 */
export function swipeEnd(_event) {
    const _ = this;
    _.dragging = false;
    _.swiping = false;
    if (_.scrolling) {
 ...
 */
export function checkNavigable(index) {
    const _ = this;
 ...
 */
export function getNavigableIndexes() {
    const _ = this;
    let breakPoint = 0;
    let counter = 0;
    const indexes = [];
